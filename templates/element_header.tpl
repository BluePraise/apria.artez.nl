...
{foreach $mainMenu as $aItem}
				<li class="menu-item"{if $color} style="color:{$color}"{/if}><a href="{$aItem->url}">{$aItem->title}</a></li>
{/foreach}
				<li class="menu-item"{if $color} style="color:{$color}"{/if}><a href="{$newsletter}" class="js-open-as-overlay">{#menu_subscribe#}</a></li>
{if $currentIssue}
